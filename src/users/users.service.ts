...
export class UsersService {
  ...
  async create(
    ...
    try {
      ...
      if (!password || !password.trim()) {
        throw new BadRequestException('A senha do usuário é obrigatória.');
      }

      this.validateAssignableRole(data.role);

      if (requesterRole === UserRole.ADMIN && data.role === UserRole.ADMIN) {
      ...
      this.handleError(error, 'Erro interno ao criar usuário.');
    }
  }
  ...
  async list(companyId: string) {
    try {
      ...
      this.handleError(error, 'Erro interno ao listar usuários.');
    }
  }
  ...
  async findById(id: string, companyId: string) {
    try {
      ...
      this.handleError(error, 'Erro interno ao buscar o usuário.');
    }
  }
  ...
  async update(
    ...
    try {
      ...
      this.handleError(error, 'Erro interno ao atualizar usuário.');
    }
  }
  ...
  async remove(
    ...
    try {
      ...
      this.handleError(error, 'Erro interno ao remover usuário.');
    }
  }

  private validateAssignableRole(role: UserRole) {
    if (role === UserRole.OWNER) {
      throw new BadRequestException('Não é permitido atribuir a role OWNER.');
    }

    if (role !== UserRole.ADMIN && role !== UserRole.OPERATOR) {
      throw new BadRequestException('A role informada é inválida.');
    }
  }
